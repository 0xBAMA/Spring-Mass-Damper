...
void graph::draw_links(SDL_Renderer* r)
{
    ...
    for(uint i = 0; i < edges.size(); i++)
    {
        ...
        if(dist > edges[i].base_length)
            SDL_SetRenderDrawColor(r, 10, 50, 255, 0xff);    //edge is in tension, draw blue 
        else
            SDL_SetRenderDrawColor(r, 255, 0, 0, 0xff);    //edge is in compression, draw red
    ...
    for(uint i = 0; i < nodes.size(); i++)
    {
        if(!nodes[i].anchor)
        {
            ...
            for(uint j = 0; j < nodes[i].edges.size(); j++) //loop through connections
            {
                ...
                glm::vec3 ur_position;

                if(nodes[nodes[i].edges[j].node2].anchor)
                    ur_position = nodes[nodes[i].edges[j].node2].position;
                else
                    ur_position = nodes[nodes[i].edges[j].node2].old_position;
                ...
                glm::vec3 springforce = -k * (my_position - ur_position) * (spring_ratio - 1);
            ...
            //compute acceleration that will come from this force, f=ma so a = f/m
        ...
        else
        {
            ...
            location += glm::vec3(0.004, 0.017, 0.003);

            float offset = p.noise(location.x + nodes[i].position.x, location.y, location.z);

            nodes[i].position = nodes[i].old_position + glm::vec3(0, 50*offset, 0);
        }
    }
}
